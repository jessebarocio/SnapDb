...
Task NuGetPack -depends ReleaseBuild{
	...
	NuGetPackCsproj .\SnapDb
}

Task CodeCoverage {
    # Run OpenCover
    md "$baseDir\TestData\CodeCoverage" -Force
    $user = $env:username
    cmd /c "$openCoverDir\OpenCover.Console.exe -target:$nunitDir\nunit-console-x86.exe -targetargs:`"SnapDb.Tests.dll /noshadow /exclude:`"Spike`" `" -targetdir:.\SnapDb.Tests\bin\Release -output:.\TestData\CodeCoverage\results.unit.xml -filter:`"+[SnapDb]* -[SnapDb]*Exception`" -register:user"
    # Run ReportGenerator
    exec { & $reportGeneratorDir\ReportGenerator.exe -reports:"$baseDir\TestData\CodeCoverage\Results.*.xml" -targetdir:$baseDir\TestData\CodeCoverage\Reports -reporttypes:Html }
}
